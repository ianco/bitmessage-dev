...
configuration, suitable for development and local testing.

Note that you will need Docker installed, you can get Docker [here](https://docs.docker.com/engine/installation/), along with instructions to install on your platform.
...
Another note - I run this in a VM so I can disable external networking.  Although the code is configured for a local network, the PyBitmessage client still seems to "reach out" to external peers.  I prefer to disable the external network just to be sure to keep a clean local test network.

The resulting Docker image is configured to run a local network.  The config files are stored o the host, and mounted on each container to specify a local "known node" and to disable the DNS lookup for external hosts.  However you will need to make one code change: edit "defaultknownnodes.py", comment out the "known nodes" and add the IP for your local instance, like so:
...
- The config files are copied to a new directory where the container can mount them (the container adds files and updates the config files)
...
- There are different versions of "keys.dat" in each config folder, one for each Docker container - they will expose ports 8444 and 8555 respectively (and 8666 if you configure a third container)
...
Now startup "bob" - it should connect to alice and both bob and alice should show green (or yellow) and one peer connection:
